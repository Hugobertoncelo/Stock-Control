...
export async function GET(request: NextRequest) {
  try {
    ...
    const days = parseInt(searchParams.get("dias") || "30");
    ...
    sales.forEach((sale) => {
      ...
      const cost = sale.costOfGoodsSold
    ...
    console.error("Erro ao obter dados de lucro/prejuízo:", error);
    return NextResponse.json(
      { success: false, error: "Falha ao obter dados de lucro/prejuízo" },
      { status: 500 }
    );
  }
}
